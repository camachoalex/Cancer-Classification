function [accuracy,class_of_test] = ShrunkenCentroid(xtrain,xtest, lambda)
    ...
    x_i = sum(newX')/n; %component of overall centroid, shit is also a row vector 
    ...
    temp(:,~class) = x_i2'*ones(1, n_two); % same shit as above
    S =  (1/(n-2))*(newX-temp).^2;
    s_i_square = sum(S'); %this is the fucking s_i square in the paper, row vector bitches
    m_1 = sqrt(1/n_one - 1/n); m_2 = sqrt(1/n_two - 1/n); %easy to see what this is
    d_i1 = (x_i1 - x_i)'./(m_1*sqrt(s_i_square)');  %easy to see what this is too, column vector
    d_i2 = (x_i2 - x_i)'./(m_2*sqrt(s_i_square)');

    %use cv to choose lambda
    s_i = sqrt(s_i_square);
 
    d_i1_new = sign(d_i1).*subplus((abs(d_i1)-lambda)); % compute new shit, column vec
    d_i2_new = sign(d_i2).*subplus((abs(d_i2)-lambda)); % good shit
    x_i1_new = x_i' + m_1*s_i'.*d_i1_new; %column vector
    x_i2_new = x_i' + m_2*s_i'.*d_i2_new;

    %Given a test vector x of length p, say
    test = xtest(2:end,:); 
    test_y = xtest(1,:);
    % We need to compute its discriminant score for class k to classify it
    class_of_test=[];
    for j = 1:length(test_y)
        delta_1 = sum(((test(:,j)'-x_i1_new').^2)./s_i_square) - 2*log(n_one/n);
        delta_2 = sum(((test(:,j)'-x_i2_new').^2)./s_i_square) - 2*log(n_two/n);
        %now we classify it
        if delta_1 < delta_2
            class_of_test(j) = 1;
        else
